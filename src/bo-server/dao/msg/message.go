...
FROM message WHERE receiver_id=%d AND stat=%d AND msg_type BETWEEN %d AND %d ORDER BY create_time DESC LIMIT 10`, uid, stat, IM_TEXT_MSG, IM_PTT_MSG)
	rows, err := db.Query(sql)
	if err != nil {
...
func SetMsgStat(msgID int32, stat int, db *sql.DB) error {
	...
	stmtUpd, err := db.Prepare("UPDATE message SET stat=? WHERE receiver_id=?")
	if err != nil {
		logger.Errorf("UPDATE message prepare stat error: %+v", err)
		return err
	}

	_, err = stmtUpd.Exec(stat, msgID)
	if err != nil {
		logger.Errorf("UPDATE message stat error: %+v", err)
		return err
	}
	...
	//defer res.Close()
	//if err != nil {
	//	log.log.Debugf("query(%s), error(%s)\n", sql, err)
	//	return err
	//}
	return nil
}
