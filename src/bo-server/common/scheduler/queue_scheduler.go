...
type queuedScheduler struct {
	requestChan chan interface{}		// Request channel
...
func (s *queuedScheduler) Submit(request interface{}) {
	s.requestChan <- request
}

func (s *queuedScheduler) ConfigMasterWorkerChan(chan interface{}) {
	panic("implement me")
}

// 告诉外界有一个 worker 可以接收 request
func (s *queuedScheduler) WorkerReady(w chan interface{}) {
	s.workerChan <- w
}
