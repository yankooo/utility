...
func newConsumerClient(receiverQueue chan interface{}) *consumerClient {
	var (
	...
	cc.config.ClientID = "GRPC_consumer"
	...
	cc.consumer, err = sarama.NewConsumer(conf.ConsumerAddrs, cc.config)
	...
	cc.client, err = sarama.NewClient(conf.ConsumerAddrs, cc.config)
	if err != nil {
	...
	cc.offsetManager, err = sarama.NewOffsetManagerFromClient(conf.ConsumerGroup, cc.client)
	if err != nil {
	...
	cc.partitionOffsetManager, err = cc.offsetManager.ManagePartition(conf.ConsumerTopic, conf.ConsumerPartition)
	if err != nil {
	...
	partitions, _ := cc.consumer.Partitions(conf.ConsumerTopic)
	...
	cc.partitionConsumer, err = cc.consumer.ConsumePartition(conf.ConsumerTopic, conf.ConsumerPartition, nextOffset+1)
	if err != nil {
		logger.Fatalf("newConsumerClient cc.offsetManager.ManagePartition fail : %+v", err)
		return nil
	}
